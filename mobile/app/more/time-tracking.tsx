...
type TabKey = 'timer' | 'sheet' | 'payroll' | 'stats';
...
type PayrollPeriod = 'this_week' | 'last_week' | 'this_fortnight' | 'this_month' | 'last_month';

const PAYROLL_PERIODS: { key: PayrollPeriod; label: string }[] = [
  { key: 'this_week', label: 'This Week' },
  { key: 'last_week', label: 'Last Week' },
  { key: 'this_fortnight', label: 'This Fortnight' },
  { key: 'this_month', label: 'This Month' },
  { key: 'last_month', label: 'Last Month' },
];

function getPayrollDates(period: PayrollPeriod) {
  const now = new Date();
...
const createStyles = (colors: ThemeColors) => StyleSheet.create({
  ...
  payrollPeriodOptionActive: {
    backgroundColor: `${colors.primary}15`,
  },
  payrollPeriodOptionText: {
    fontSize: 15,
    color: colors.foreground,
  },
  payrollPeriodOptionTextActive: {
    color: colors.primary,
    fontWeight: '500',
  },
  payrollLoadingContainer: {
    alignItems: 'center',
  ...
  payrollWorkersSection: {
    gap: spacing.sm,
  },
  payrollWorkersSectionTitle: {
    fontSize: 12,
    fontWeight: '600',
    color: colors.mutedForeground,
    letterSpacing: 0.5,
  ...
  payrollWorkerPayAmount: {
    fontSize: 16,
    fontWeight: 'bold',
    color: colors.foreground,
  },
  payrollWorkerDetails: {
    flexDirection: 'row',
    flexWrap: 'wrap',
    gap: spacing.sm,
  ...
  payrollBillableBadge: {
    paddingHorizontal: 6,
    paddingVertical: 2,
    borderRadius: radius.sm,
  },
  payrollBillableBadgeText: {
    fontSize: 10,
    fontWeight: '600',
  },
  payrollApprovalRow: {
...
export default function TimeTrackingScreen() {
  ...
  // Payroll State
  const [payrollPeriod, setPayrollPeriod] = useState<PayrollPeriod>('this_month');
  const [payrollData, setPayrollData] = useState<any>(null);
  const [isLoadingPayroll, setIsLoadingPayroll] = useState(false);
  const [showPayrollPeriodPicker, setShowPayrollPeriodPicker] = useState(false);
  ...
  const fetchPayrollData = useCallback(async () => {
    ...
    try {
      const dates = getPayrollDates(payrollPeriod);
      const response = await api.get<any>(
        `/api/payroll/summary?start=${dates.start.toISOString()}&end=${dates.end.toISOString()}`
      );
      if (response.data) {
        setPayrollData(response.data);
      }
    } catch (error) {
      console.log('Failed to fetch payroll data:', error);
    } finally {
        ...
        >
          ...
          {activeTab === 'payroll' && (
            <View style={styles.payrollSection}>
              <TouchableOpacity
                style={styles.payrollPeriodSelector}
                onPress={() => setShowPayrollPeriodPicker(!showPayrollPeriodPicker)}
              >
                <Feather name="calendar" size={18} color={colors.primary} />
                <Text style={styles.payrollPeriodText}>
                  {PAYROLL_PERIODS.find(p => p.key === payrollPeriod)?.label}
                </Text>
                <Feather name={showPayrollPeriodPicker ? "chevron-up" : "chevron-down"} size={18} color={colors.mutedForeground} />
              </TouchableOpacity>

              {showPayrollPeriodPicker && (
                <View style={styles.payrollPeriodPicker}>
                  {PAYROLL_PERIODS.map((p, index) => (
                    <TouchableOpacity
                      key={p.key}
                      style={[
                        styles.payrollPeriodOption,
                        payrollPeriod === p.key && styles.payrollPeriodOptionActive,
                        index === PAYROLL_PERIODS.length - 1 && { borderBottomWidth: 0 }
                      ]}
                      onPress={() => {
                        setPayrollPeriod(p.key);
                        setShowPayrollPeriodPicker(false);
                      }}
                    >
                      <Text style={[
                        styles.payrollPeriodOptionText,
                        payrollPeriod === p.key && styles.payrollPeriodOptionTextActive
                      ]}>
                        {p.label}
                      </Text>
                    </TouchableOpacity>
              ...
              {isLoadingPayroll ? (
                <View style={styles.payrollLoadingContainer}>
                  <ActivityIndicator size="large" color={colors.primary} />
                  <Text style={styles.payrollLoadingText}>Loading payroll data...</Text>
                </View>
              ) : payrollData ? (
                <>
                  <View style={styles.payrollSummaryGrid}>
                    <View style={styles.payrollSummaryCard}>
                      ...
                      <Text style={styles.payrollSummaryValue}>
                        {(payrollData.summary?.totalHours || 0).toFixed(1)}h
                      </Text>
                      <Text style={styles.payrollSummaryLabel}>Total Hours</Text>
                    </View>
                    <View style={styles.payrollSummaryCard}>
                      <Feather name="dollar-sign" size={18} color={colors.success} />
                      <Text style={styles.payrollSummaryValue}>
                        ${(payrollData.summary?.totalGrossPay || 0).toLocaleString('en-AU', { minimumFractionDigits: 2 })}
                      </Text>
                      <Text style={styles.payrollSummaryLabel}>Total Pay</Text>
                    </View>
                    <View style={styles.payrollSummaryCard}>
                      <Feather name="users" size={18} color={colors.info} />
                      <Text style={styles.payrollSummaryValue}>
                        {payrollData.workers?.filter((w: any) => !w.isSubcontractor).length || 0}
                      </Text>
                      <Text style={styles.payrollSummaryLabel}>Employees</Text>
                    </View>
                    <View style={styles.payrollSummaryCard}>
                      ...
                      <Text style={styles.payrollSummaryLabel}>Subcontractors</Text>
                    </View>
                  </View>

                  <View style={styles.payrollWorkersSection}>
                    <Text style={styles.payrollWorkersSectionTitle}>TEAM MEMBERS</Text>
                    {payrollData.workers && payrollData.workers.length > 0 ? (
                      payrollData.workers
                        ...
                        .map((worker: any, index: number) => {
                          const name = `${worker.firstName || ''} ${worker.lastName || ''}`.trim() || 'Unknown';
                          const initials = ((worker.firstName?.[0] || '') + (worker.lastName?.[0] || '')).toUpperCase() || '?';
                          const billableHours = worker.billableHours || 0;
                          const totalHours = worker.totalHours || 0;
                          const billablePct = totalHours > 0 ? Math.round((billableHours / totalHours) * 100) : 0;

                          return (
                            <View key={worker.id || worker.userId || `worker-${index}`} style={styles.payrollWorkerCard}>
                              <View style={styles.payrollWorkerHeader}>
                                <View style={styles.payrollWorkerAvatar}>
                                  <Text style={styles.payrollWorkerAvatarText}>{initials}</Text>
                                </View>
                                <View style={styles.payrollWorkerInfo}>
                                  <Text style={styles.payrollWorkerName}>{name}</Text>
                                  <Text style={styles.payrollWorkerType}>
                                    {worker.isSubcontractor ? 'Subcontractor' : 'Employee'}
                                    {worker.hourlyRate > 0 ? ` \u00B7 $${worker.hourlyRate.toFixed(2)}/hr` : ''}
                                  </Text>
                                </View>
                                <View style={styles.payrollWorkerPay}>
                                  <Text style={styles.payrollWorkerPayAmount}>
                                    ${(worker.grossPay || 0).toLocaleString('en-AU', { minimumFractionDigits: 2 })}
                                  </Text>
                                </View>
                              </View>

                              <View style={styles.payrollWorkerDetails}>
                                <View style={styles.payrollWorkerDetail}>
                                  <Text style={styles.payrollDetailLabel}>Regular</Text>
                                  <Text style={styles.payrollDetailValue}>{(worker.regularHours || 0).toFixed(1)}h</Text>
                                </View>
                                {(worker.overtimeHours || 0) > 0 && (
                                  <View style={styles.payrollWorkerDetail}>
                                    <Text style={[styles.payrollDetailLabel, { color: colors.warning }]}>Overtime</Text>
                                    <Text style={[styles.payrollDetailValue, { color: colors.warning }]}>{worker.overtimeHours.toFixed(1)}h</Text>
                                  </View>
                                )}
                                <View style={styles.payrollWorkerDetail}>
                                  <Text style={styles.payrollDetailLabel}>Billable</Text>
                                  <View style={styles.payrollBillableRow}>
                                    <Text style={styles.payrollDetailValue}>{billableHours.toFixed(1)}h</Text>
                                    <View style={[
                                      styles.payrollBillableBadge,
                                      { backgroundColor: billablePct >= 80 ? `${colors.success}20` : billablePct >= 50 ? `${colors.warning}20` : `${colors.destructive}20` }
                                    ]}>
                                      <Text style={[
                                        styles.payrollBillableBadgeText,
                                        { color: billablePct >= 80 ? colors.success : billablePct >= 50 ? colors.warning : colors.destructive }
                                      ]}>{billablePct}%</Text>
                                    </View>
                                  </View>
                                </View>
                                <View style={styles.payrollWorkerDetail}>
                                  <Text style={styles.payrollDetailLabel}>Non-Billable</Text>
                                  <Text style={styles.payrollDetailValue}>{(totalHours - billableHours).toFixed(1)}h</Text>
                                </View>
                              </View>

                              {(worker.approved > 0 || worker.unapproved > 0) && (
                                <View style={styles.payrollApprovalRow}>
                                  {worker.approved > 0 && (
                                    <View style={[styles.payrollApprovalBadge, { backgroundColor: `${colors.success}20` }]}>
                                      <Feather name="check-circle" size={12} color={colors.success} />
                                      <Text style={[styles.payrollApprovalText, { color: colors.success }]}>{worker.approved} approved</Text>
                                    </View>
                                  )}
                                  {worker.unapproved > 0 && (
                                    <View style={[styles.payrollApprovalBadge, { backgroundColor: `${colors.warning}20` }]}>
                                      <Feather name="alert-circle" size={12} color={colors.warning} />
                                      <Text style={[styles.payrollApprovalText, { color: colors.warning }]}>{worker.unapproved} pending</Text>
                                    </View>
                                  )}
                                </View>
                              )}
                      ...
                      <View style={styles.emptyState}>
                        ...
                        <Text style={styles.emptyStateText}>No worker data for this period</Text>
                      </View>
                    )}
                  </View>
                </>
                ...
                <View style={styles.emptyState}>
                  ...
                  <Text style={styles.emptyStateText}>No payroll data available</Text>
                </View>
              )}
            </View>
          )}
