...
*   **Visual Dispatch Board**: Three views — Schedule (timeline/calendar), Kanban Board (5-column status board), and Map View (Leaflet with job/worker pins). Uses `/api/dispatch/board` for unified data. Includes Ops Health Banner (persistent alert strip showing conflicts, unassigned jobs, overdue items, over-capacity workers, overdue invoices with severity-based styling). Schedule conflict detection with visual warnings (red rings on overlapping job cards, drag-drop conflict warnings). Best-available worker suggestions in unscheduled jobs sidebar. Backend `/api/ops/health` endpoint for operational metrics.
...
*   **Equipment Management**: Full CRUD for equipment registry, categories, and maintenance logs with status tracking. Job-equipment assignments via `job_equipment` join table with wrench indicators on Schedule cards and "Assigned to Jobs" in equipment detail sheets. Equipment utilisation reporting with hours/km tracking per job, capacity used vs available, oversized flags, post-job notes, and utilization rate calculations. Utilisation tab in Equipment page with KPI summary and expandable equipment cards.
*   **Deep-Link Navigation**: Dashboard KPI widgets, Action Centre items, and Insights metric cards all navigate to specific filtered views (e.g., clicking "Active Jobs" opens Work page pre-filtered to in-progress). WorkPage, DocumentsHub read URL query params (?filter=, ?tab=, ?action=) to restore state from deep links.
