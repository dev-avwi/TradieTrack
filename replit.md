...
-   **UI/UX**: Mobile-first design with a native app feel, card-based layouts, touch-optimized components, and a "Today's Schedule" dashboard. Includes features like Quick Add Client (on both web and mobile quote/invoice creation screens), Enhanced Template Selector, Smart Address Auto-fill, and Contextual Quote/Invoice Creation.
...
-   **Role-Based Access Control (RBAC)**: Granular permissions and role templates (OWNER, ADMIN, MANAGER, SUPERVISOR, STAFF) enforced by middleware. Job assignment follows hierarchy: Owner assigns to anyone, Manager/Admin assigns to workers only. Job unassignment supported via `POST /api/jobs/:id/assign` with `{ assignedTo: null }`. Supports role normalization for common tradie titles (Technician, Tradesman, Apprentice, etc.).
...
-   **Mobile App (React Native/Expo)**: Full API integration, Zustand for state management, SQLite-based offline mode with background sync (15-min intervals), conflict resolution UI, quote/invoice line items caching, max 10 retry attempts with exponential backoff, and permission-aware navigation. Uses EAS development build for native module support (expo-av for voice recording). Native email integration via Linking.openURL for quote/invoice sending (opens Gmail/Outlook directly so users can customize before sending). AI Photo Analysis with SSE streaming support—uses fetch with ReadableStream reader for real-time text display with typing cursor; includes "Add to Notes" and "Discard" actions, respects aiEnabled/aiPhotoAnalysisEnabled business settings. Team Management screen (`mobile/app/more/team-management.tsx`) with full feature parity to web: location sharing toggle (locationEnabledByOwner), resend invite for pending members, assign job modal, permissions editing, role management, and member removal. Beta mode (BETA_MODE constant) allows all users to access team features during testing. Mobile Integrations screen (`mobile/app/more/integrations.tsx`) with Stripe Connect, Xero, and SMS integration cards.
-   **Mobile Xero Integration**: OAuth flow via web deep link - mobile app fetches connect URL from `/api/integrations/xero/connect-link`, opens web browser for OAuth, user returns to mobile and refreshes to see connection status. Shows sync controls (push contacts/invoices) and disconnect option when connected.
-   **Mobile SMS Integration**: Demo mode with test templates for users without Twilio credentials. SMS preview endpoint at `/api/integrations/test-sms-preview` returns appointment reminder, on-the-way, job complete, and payment received templates with business name. Shows actual Twilio status (Connected vs Demo Mode) from integration health endpoint.
