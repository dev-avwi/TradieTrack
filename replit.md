...
- **Mobile Map Animations & Real-Time Tracking**: Added Life360-level polish with smooth camera animations (500ms duration), haptic feedback on tap, spring-based marker scale animation (1→1.2→1) on selection, and real-time location polling every 10 seconds with "LIVE" badge indicator. Fixed top-left flash bug by filtering invalid/zero coordinates at data transformation step using Number.isFinite validation. Optimized polling interval cleanup to prevent overlapping timers.
- **Mobile Map Zoom Persistence**: Fixed issue where map would auto-zoom out after 3 seconds - added user interaction tracking (`userHasInteractedRef`) to prevent auto-fit after user manually pans/zooms. Auto-fit now only runs on initial load or when filters are explicitly toggled.
- **Mobile Map Marker Stability**: Fixed marker jittering/glitching when zoomed in by adding 5-meter movement threshold using Haversine formula. Team member coordinates only update when they've moved significantly, preventing visual jitter from GPS noise.
- **Web/Mobile Map Data Parity**: Enhanced `/api/team/locations` endpoint with additional Life360-style fields (profileImageUrl, themeColor, speed, batteryLevel, heading) to match web `/api/map/team-locations` endpoint.
- **AI Voice Note Transcription (Mobile)**: Added AI transcription button to voice note player with "Add to Notes" and "Copy" quick actions. Uses OpenAI Whisper via `/api/jobs/:jobId/voice-notes/:voiceNoteId/transcribe` endpoint.
- **Mobile Map Team Marker Flash Fix**: Eliminated the iOS react-native-maps bug where team markers briefly flash at screen position (0,0) before rendering correctly. Added `teamMarkersReady` state with 150ms delay after data fetch to ensure coordinates are fully processed before rendering. Both map markers and team chips are now gated by this ready state.
- **Mobile Job Assignment Fix**: Fixed "response.json is not a function" error when assigning jobs to team members - corrected API response handling to use `{ data, error }` format instead of fetch Response methods.