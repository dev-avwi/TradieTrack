...
*   **Mobile Parity (React Native/Expo)**: Full feature parity for: Material status tracking (needed/ordered/shipped/received/installed) with color-coded badges and status picker, cost prompt modal when marking materials as received/installed without cost data, "Cost: Not set" summary display, Smart Payment Chaser tab (KPIs, AI advisor, chase queue with urgency levels, tone-selected reminders, call/email shortcuts), Chat Hub SMS scoping fix (clientsWithSmsAssigned prevents duplicate conversations), and SMS quick action templates (On my way, Running late, Job done).
