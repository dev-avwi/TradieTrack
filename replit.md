...
-   **Role-Based Access Control (RBAC)**: Granular permissions and role templates (OWNER, ADMIN, MANAGER, SUPERVISOR, STAFF) enforced by middleware. Job assignment follows hierarchy: Owner assigns to anyone, Manager/Admin assigns to workers only. Supports role normalization for common tradie titles (Technician, Tradesman, Apprentice, etc.).
