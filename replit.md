...
*   **Email Delivery Tracking (SendGrid Event Webhook)**: All transactional emails (quotes, invoices, receipts, reminders) sent via SendGrid have open + click tracking enabled by default and a per-message `delivery_log_id` custom arg. The `POST /api/webhooks/sendgrid` endpoint (registered before `express.json` with `express.raw`) verifies the ECDSA secp256r1 Signed Event Webhook signature using `SENDGRID_WEBHOOK_PUBLIC_KEY` (the verification key copied from the SendGrid Mail Settings → Event Webhook page), then asynchronously updates `email_delivery_logs` with `openedAt`, `clickedAt`, `bouncedAt`, `bounceReason`, `lastEventType`, `lastEventAt`, `openCount`, `clickCount`. Correlation prefers the custom `delivery_log_id` arg (set via `_meta.deliveryLogId` plumbed through `sendEmail` → `sendSystemEmail` → `sendViaSendGrid`) and falls back to matching on the X-Message-Id base (SendGrid suffixes events as `<base>.<filterdrecv>`). The `GET /api/email-delivery-logs?type=&relatedId=` endpoint returns tracking rows for the user's tenant; the Communications Hub UI fetches it on a 30s interval and renders blue "Opened ×N", green "Clicked ×N", and red "Bounced" badges next to each sent email card.
...
*   **Unified Team & Magic-Link Subcontractor Flow**: Single `/team` page with Members + Subcontractors tabs replacing the old 5-entry sidebar group. Subcontractor table merges three kinds (account subs, magic-link, connected businesses) via `GET /api/subcontractors`. Magic-link send flow defaults wrong-number defence ON (4-digit verification code, contractor reads code separately). PII-free SMS template ("A contractor has sent you a job"); name-confirmation gate at `/m/:token` reveals only first character of recipient name; legacy `/s/:token` info/request-code/verify-code endpoints reject magic-link tokens to prevent bypass. Verification code uses `crypto.randomInt`, sha256-hashed, atomic SQL increment of attempts (3 then auto-revoke), 30-min TTL fail-closed, per-token+IP rate-limited (5/5min). Centralised `validateSubSession` helper guards all sub web-view endpoints (data/accept/decline/status/notes/photos/location) — sessions die instantly on token revoke/expire. New columns on `subcontractor_tokens`: requireCode, codeHash, codeAttempts, codeIssuedAt, nameConfirmedAt, lastOpenedFromCity, lastOpenedFromIp, openCount, revokedReason, recipientUserId. Endpoints: `/api/m/:token/{preview,confirm,verify,revoke,account-status,save-account}`.
*   **Magic-link → Account Sub Upgrade (T037)**: Owners on the Team plan can upgrade an existing magic-link sub to a full account sub from the Subcontractors tab on `/team` via a row action ("Upgrade to account sub"). Server endpoint `POST /api/subcontractors/upgrade-to-account` (gated `requireAuth + ownerOrManagerOnly + requireTeamPlan`) accepts `{tokenId, email, firstName?, lastName?, hourlyRate?, roleId?}`, looks up the token by `id+userId`, resolves/creates the Subcontractor role, creates a `team_members` row (re-using contact name + phone from the token), and sends both an email + SMS account-creation invite. Idempotent on existing email under same owner. The `/team` page Add chooser also opens real in-page invite sheets for "Add member" and "Add account sub" (replacing the previous redirect-to-team-operations behavior); the Subcontractor role is hidden from the member-invite role select. Legacy team page files (`TeamDashboard.tsx`, `TeamManagement.tsx`, `TeamGroups.tsx`) deleted; `/team-dashboard`, `/team-management`, `/team-groups` now redirect to `/team`.
*   **Sub Onboarding for Existing Accounts (T008)**: When a contractor invites a phone number that already belongs to a JR user (any role), the token is auto-linked at create time — `recipientUserId` and `nameConfirmedAt` are set on the row, so the recipient skips the "Are you Jake?" gate and lands directly on code-entry. Phone matching uses E.164 normalization (`+61` prefix), with `users.phoneNormalized` indexed via partial unique-style index. After the sub completes a job, a premium blue CTA card on the dashboard offers either "Link contractor" (existing user) or "Create free account" (new user), opening a 3-phase bottom sheet (`SaveAccountSheet`). New users get a free-tier account with phone+phoneNormalized populated via `POST /api/m/:token/save-account`; the same endpoint also retroactively links every other unrevoked token sent to that phone so future invites from any contractor skip the gate. Activity log fires `SUBBIE_NAME_CONFIRMED` at auto-link, `SUBBIE_ACCOUNT_CREATED` / `SUBBIE_ACCOUNT_LINKED` at save. **Security**: the unauthenticated `/preview` endpoint never returns the recipient's first name — personalized "Welcome back, X" only renders post-session via the gated `/account-status` endpoint, preventing pre-auth PII enumeration.
...
*   **Track My Arrival (Job Portal worker presentation + SMS polish)**: The public job portal at `/p/:token` shows the assigned worker(s) with their `users.profileImageUrl` photo when present, falling back to coloured initials in the WORKER_COLORS palette. Photo URL is propagated through three paths in `GET /api/public/job-portal/:token`: assignment-scoped `workerInfo`, multi-worker `assignmentsArray`, and the legacy `assignedTo` team-member fallback (which fetches `users.profileImageUrl` via `teamMembers.memberId`). Helmet `imgSrc` widened to `https:` so any object-storage / SSO avatar host loads without CSP errors. SMS templates polished in `assignmentWorkflowService.ts`: `handleDelayedNotification` now opens with an apology and includes both the live tracking link and owner phone (`"<Business>: Apologies — <Worker> is running ~N mins late. Track live: <portalUrl>. Questions? Call <ownerPhone>."`). Cross-status anti-spam added in `handleWorkerStatusChange`: if an `arrived` SMS was sent in the last 30 minutes, the subsequent `in_progress` SMS is suppressed (workers commonly press Arrived → Start Work back-to-back; clients no longer get two near-identical texts).
