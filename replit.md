...
-   **Life360-Style Team Hub** (`/team-dashboard`): Legacy route - now redirects to Team Operations. Interconnected team experience with 3-panel layout showing Team Status Board (presence dots, member status), Activity Feed timeline (job/quote/invoice events), and Team Map (live locations). Includes status selector (Online/Busy/On Job/Break/Offline) and 30-second heartbeat for real-time presence tracking.
