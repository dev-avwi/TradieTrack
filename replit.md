...
*   **SWMS (Safe Work Method Statements)**: Full Australian WHS-compliant SWMS system. Schema: `swms_documents`, `swms_hazards`, `swms_signatures` tables. Features: 9 pre-built trade-specific templates (Heights, Electrical, Excavation, Demolition, Confined Space, Hot Work, Roofing, Plumbing, General Construction), 5x5 risk matrix (likelihood x consequence), 10 PPE categories, worker sign-off with GPS capture, PDF generation via Puppeteer. Web: SwmsBuilder component (`client/src/components/SwmsBuilder.tsx`) integrated into SafetyFormsSection on job detail page. Mobile: Safety tab on job detail (`mobile/app/job/[id].tsx`) with template picker, SWMS creation, hazard assessment, PPE selection, and worker signing. Proof Pack integration: SWMS section (Section 9/10) showing hazard tables, PPE, and worker signatures with toggle `hide_swms`. APIs: `GET/POST /api/swms`, `GET/PATCH/DELETE /api/swms/:id`, `GET /api/jobs/:jobId/swms`, `POST /api/swms/:id/hazards`, `PATCH/DELETE /api/swms/hazards/:hazardId`, `POST /api/swms/:id/sign`, `GET /api/swms/:id/pdf`, `GET /api/swms/templates`, `GET /api/swms/templates/:id`, `GET /api/swms/risk-matrix`.
...
*   **Photo Organiser**: Central photo library on the Files page (Photos tab) with stats strip (Total, This Week, Top Job, Storage), category filters (All/Before/During/After/Materials/General), job/client dropdowns, date-grouped grid with thumbnails, lightbox detail view, clickable job/client links for navigation, bulk actions (attach to job, recategorise, tag, delete), standalone upload modal with drag-and-drop/camera/gallery support, 25MB max file size, MIME type validation. Schema: `job_photos.tags text[]` column, `jobId` nullable for standalone photos. APIs: `GET /api/photos` (with filters/sort/join), `GET /api/photos/stats`, `GET /api/photos/tags`, `PATCH /api/photos/:id`, `PATCH /api/photos/bulk`, `POST /api/photos/upload-standalone`. Web component: `client/src/components/PhotoLibraryTab.tsx`. **Mobile parity**: Full native Photo Library in `mobile/src/components/PhotoLibrary.tsx`, integrated into mobile Files page (`mobile/app/more/files.tsx` Photos tab). Features: stats strip, category filter pills, 3-column photo grid with date grouping, full-screen lightbox with prev/next navigation and inline editing (category/caption/tags), job/client deep-links, native camera + gallery upload via expo-image-picker with base64 encoding via expo-file-system, upload progress bar, long-press bulk selection mode with bulk delete/recategorise/attach-to-job actions.
