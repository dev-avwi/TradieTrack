...
-   **Mobile App (React Native/Expo)**: Full API integration, Zustand for state management, SQLite-based offline mode with background sync, and permission-aware navigation. Supports native email integration and AI Photo Analysis with streaming. Team Management and Integrations screens mirror web functionality.
    ...
    - **iPad Sidebar Layout**: Responsive tablet detection using Platform.isPad with 768px width fallback. iPad displays a 280px sidebar (matching web app navigation structure) with business branding, main nav items (Dashboard, Jobs, Chat, Map, Money, Clients, Schedule, Reports), settings section (Team, Integrations, Settings), and user profile footer. iPhone maintains bottom tab navigation. Device type detection in `mobile/src/lib/device.ts`, navigation config in `mobile/src/lib/navigation-config.ts`, SidebarNav component in `mobile/src/components/SidebarNav.tsx`.
