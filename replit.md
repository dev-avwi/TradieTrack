...
*   **Smart Payment Chaser**: Integrated into the Payment Hub as a dedicated tab. Provides AI-powered payment collection advice, prioritised chase queue with urgency levels (Friendly/Firm/Final Notice/Critical), chase history tracking, one-tap reminder sending with tone selection (friendly/professional/firm), and client contact shortcuts (call/email). KPI dashboard shows total overdue, clients to chase, average days overdue, and collection rate. Uses batch query for reminder logs (single `inArray` query instead of N+1 per-invoice queries) for reliability.
