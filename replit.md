...
*   **WHS Safety Hub**: Comprehensive Work Health & Safety module based on White Card training (CPCCWHS1001). Includes Incident Reports (6 types, 4 severities with witness/action tracking), Hazard Reports (proactive hazard reporting with risk levels, location, recommended actions, supervisor reporting — matching White Card hazard report form), Site Emergency Plans (assembly points, first aid, fire equipment, evacuation routes), JSA Builder (step-based Job Safety Analysis with hazards/controls per step), Hazardous Environment tracking (13 environment types), Safety Signage management (5 sign categories), and WHS Role assignment (first aid officer, HSR, WHS committee, fire warden). DB tables: `incident_reports`, `hazard_reports`, `site_emergency_info`, `jsa_documents`, `jsa_steps`, `site_hazardous_environments`, `site_safety_signage`. API routes: `/api/whs/*`. Web: `/whs` route. Mobile: `/more/whs-hub`.
