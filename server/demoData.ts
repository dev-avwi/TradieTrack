...
export async function createDemoUserAndData() {
  try {
    ...
    if (existingClients.length === 0) {
      ...
      // User already has clients - but check if we need to create the document chain
      // This runs for existing demo users to ensure they have test data for Documents Hub
      const existingReceipts = await storage.getReceipts(demoUser.id);
      const hasReceiptWithInvoice = existingReceipts.some(r => r.invoiceId !== null);
      
      if (!hasReceiptWithInvoice) {
        console.log('🔄 Creating document chain for existing demo user...');
        
        // Get first client for the chain
        ...
        if (firstClient) {
          ...
          const completedChainJob = await storage.createJob({
            ...
            title: "Complete Bathroom Plumbing Upgrade",
            description: "Full bathroom renovation including new fixtures, pipes, and water heater installation",
            address: firstClient.address || "15 Oak Street, Cairns QLD 4870",
            status: "invoiced" as const,
          ...
          const chainQuote = await storage.createQuote({
            ...
            acceptedBy: firstClient.name
          });

          await storage.createQuoteLineItem({ quoteId: chainQuote.id, description: "New Toilet Suite (Caroma Smart Dual Flush)", quantity: "1.00", unitPrice: "650.00", total: "650.00", sortOrder: 1 });
          await storage.createQuoteLineItem({ quoteId: chainQuote.id, description: "Basin and Vanity Unit", quantity: "1.00", unitPrice: "450.00", total: "450.00", sortOrder: 2 });
          await storage.createQuoteLineItem({ quoteId: chainQuote.id, description: "Mixer Tap Set (Chrome)", quantity: "2.00", unitPrice: "180.00", total: "360.00", sortOrder: 3 });
          await storage.createQuoteLineItem({ quoteId: chainQuote.id, description: "Installation Labour (6 hours)", quantity: "6.00", unitPrice: "120.00", total: "720.00", sortOrder: 4 });

          // 3. Create a PAID invoice linked to the quote
          const chainInvoiceNumber = await storage.generateInvoiceNumber(demoUser.id);
          const chainInvoice = await storage.createInvoice({
            userId: demoUser.id,
            clientId: firstClient.id,
          ...
          await storage.createInvoiceLineItem({ invoiceId: chainInvoice.id, description: "New Toilet Suite (Caroma Smart Dual Flush)", quantity: "1.00", unitPrice: "650.00", total: "650.00", sortOrder: 1 });
          await storage.createInvoiceLineItem({ invoiceId: chainInvoice.id, description: "Basin and Vanity Unit", quantity: "1.00", unitPrice: "450.00", total: "450.00", sortOrder: 2 });
          await storage.createInvoiceLineItem({ invoiceId: chainInvoice.id, description: "Mixer Tap Set (Chrome)", quantity: "2.00", unitPrice: "180.00", total: "360.00", sortOrder: 3 });
          await storage.createInvoiceLineItem({ invoiceId: chainInvoice.id, description: "Installation Labour (6 hours)", quantity: "6.00", unitPrice: "120.00", total: "720.00", sortOrder: 4 });
          ...
          console.log('✅ DOCUMENT CHAIN CREATED FOR EXISTING USER:');
          console.log(`   Quote ${chainQuote.number} → Invoice ${chainInvoice.number} → Receipt ${chainReceipt.receiptNumber}`);
        }
      }
    }
