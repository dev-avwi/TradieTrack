...
import {
  ...
  insertBookingPortalSettingsSchema,
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ==================== Online Booking Portal Routes ====================
  
  // Get booking portal settings (authenticated)
  app.get("/api/booking/settings", requireAuth, async (req: any, res) => {
    try {
      const settings = await storage.getBookingPortalSettings(req.userId);
  ...
  // Create/Update booking portal settings
  app.post("/api/booking/settings", requireAuth, async (req: any, res) => {
    try {
      const parseResult = insertBookingPortalSettingsSchema.partial().safeParse(req.body);
      if (!parseResult.success) {
        return res.status(400).json({ error: "Invalid booking settings", details: parseResult.error.flatten() });
      }
      
      const existing = await storage.getBookingPortalSettings(req.userId);
      if (existing) {
        const updated = await storage.updateBookingPortalSettings(req.userId, parseResult.data);
        res.json(updated);
      } else {
        const created = await storage.createBookingPortalSettings(req.userId, parseResult.data);
        res.json(created);
      }
    } catch (error: any) {
  ...
  app.patch("/api/booking/settings", requireAuth, async (req: any, res) => {
    try {
      const parseResult = insertBookingPortalSettingsSchema.partial().safeParse(req.body);
      if (!parseResult.success) {
        return res.status(400).json({ error: "Invalid booking settings", details: parseResult.error.flatten() });
  ...
  // Get booking services (authenticated)
  app.get("/api/booking/services", requireAuth, async (req: any, res) => {
    try {
      const services = await storage.getBookingServices(req.userId);
      res.json(services);
    } catch (error: any) {
  ...
  app.post("/api/booking/services", requireAuth, async (req: any, res) => {
    try {
      const parseResult = insertBookingServiceSchema.safeParse(req.body);
      if (!parseResult.success) {
        return res.status(400).json({ error: "Invalid service data", details: parseResult.error.flatten() });
      }
      const service = await storage.createBookingService(req.userId, parseResult.data);
      res.json(service);
    } catch (error: any) {
  ...
  app.patch("/api/booking/services/:id", requireAuth, async (req: any, res) => {
    try {
      const { id } = req.params;
      const parseResult = insertBookingServiceSchema.partial().safeParse(req.body);
      if (!parseResult.success) {
        return res.status(400).json({ error: "Invalid service data", details: parseResult.error.flatten() });
      }
      const updated = await storage.updateBookingService(id, req.userId, parseResult.data);
      if (!updated) {
  ...
  app.delete("/api/booking/services/:id", requireAuth, async (req: any, res) => {
    try {
      const { id } = req.params;
      const deleted = await storage.deleteBookingService(id, req.userId);
  ...
  // Get booking requests (authenticated)
  app.get("/api/booking/requests", requireAuth, async (req: any, res) => {
    try {
      const requests = await storage.getBookingRequests(req.userId);
      res.json(requests);
    } catch (error: any) {
  ...
  // Update booking request (confirm/decline)
  app.patch("/api/booking/requests/:id", requireAuth, async (req: any, res) => {
    try {
      const { id } = req.params;
      ...
      if (req.body.status === 'confirmed' && request.status !== 'confirmed') {
        ...
        if (!clientId) {
          const newClient = await storage.createClient({
            ...
            address: request.clientAddress || undefined,
          });
        ...
        const job = await storage.createJob({
          clientId,
          title: request.serviceName || 'Booking Request',
          description: request.notes || '',
          status: 'scheduled',
          scheduledDate: request.preferredDate || new Date(),
          scheduledTime: request.preferredTimeSlot?.split('-')[0] || '09:00',
          address: request.clientAddress || '',
        });
        updates.jobId = job.id;
      } else if (req.body.status === 'declined' && request.status !== 'declined') {
  ...
  // ==================== Public Booking Portal Routes (No Auth) ====================
  
  // Get public booking portal by slug
  app.get("/api/public/booking/:slug", async (req, res) => {
    try {
      const { slug } = req.params;
      const settings = await storage.getBookingPortalBySlug(slug);
      
      if (!settings || !settings.isEnabled) {
        return res.status(404).json({ error: "Booking portal not found" });
      }

      // Get business info for display
      const businessSettings = await storage.getBusinessSettings(settings.userId);
      
      // Get active services
      const services = await storage.getActiveBookingServices(settings.userId);
      ...
      res.json({
        ...
        business: {
          name: businessSettings?.businessName || 'Business',
      ...
      console.error("Error getting public booking portal:", error);
      res.status(500).json({ error: error.message });
    }
  });

  // Submit public booking request (No Auth)
  app.post("/api/public/booking/:slug", async (req, res) => {
    try {
      ...
      const { serviceId, clientName, clientEmail, clientPhone, clientAddress, preferredDate, preferredTimeSlot, alternateDate, notes } = req.body;

      if (!clientName || !clientEmail) {
        return res.status(400).json({ error: "Name and email are required" });
      }

      if (settings.requirePhone && !clientPhone) {
        return res.status(400).json({ error: "Phone number is required" });
      }

      if (settings.requireAddress && !clientAddress) {
        return res.status(400).json({ error: "Address is required" });
      }

      // Validate lead time if preferred date is provided
      if (preferredDate) {
        const requestedDate = new Date(preferredDate);
        const now = new Date();
        const minLeadTime = settings.minLeadTimeHours || 24;
        ...
        if (requestedDate < minBookingTime) {
          return res.status(400).json({ error: `Bookings require at least ${minLeadTime} hours advance notice` });
        }
        ...
        if (requestedDate > maxBookingTime) {
          return res.status(400).json({ error: `Bookings cannot be made more than ${maxAdvanceDays} days in advance` });
        }
      }
        ...
        try {
          ...
          await sendEmail({
            ...
            html: `
              ...
              <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
                <div style="background: ${brandColor}; padding: 20px; border-radius: 8px; margin-bottom: 20px; text-align: center;">
                  <h1 style="color: white; margin: 0;">${businessName}</h1>
                </div>
                <h2>Thank You for Your Booking Request!</h2>
                <p>Hi ${clientName},</p>
                <p>We've received your booking request for <strong>${serviceName}</strong>.</p>
                ${preferredDate ? `<p><strong>Preferred Date:</strong> ${new Date(preferredDate).toLocaleDateString('en-AU', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>` : ''}
                ${preferredTimeSlot ? `<p><strong>Preferred Time:</strong> ${preferredTimeSlot}</p>` : ''}
                <p>We'll be in touch shortly to confirm your booking.</p>
                <p>Kind regards,<br>${businessName}</p>
              </body>
              </html>
            `,
          });
          ...
          if (settings.notifyOnBookingEmail || businessSettings?.email) {
            await sendEmail({
              to: settings.notifyOnBookingEmail || businessSettings?.email || '',
              ...
              html: `
                ...
                <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
                  ...
                  ${preferredDate ? `<p><strong>Preferred Date:</strong> ${new Date(preferredDate).toLocaleDateString('en-AU')}</p>` : ''}
                  ${preferredTimeSlot ? `<p><strong>Preferred Time:</strong> ${preferredTimeSlot}</p>` : ''}
                  ${notes ? `<p><strong>Notes:</strong> ${notes}</p>` : ''}
                  <p><a href="${process.env.REPLIT_DEV_DOMAIN ? 'https://' + process.env.REPLIT_DEV_DOMAIN : ''}/booking/requests">View Booking Requests</a></p>
                </body>
                </html>
              `,
            });
  ...
  app.get("/api/public/booking/:slug/slots", async (req, res) => {
    try {
      ...
      const availableDays = (settings.availableDays as string[]) || ['monday', 'tuesday', 'wednesday', 'thursday', 'friday'];
      ...
      const startHour = parseInt(settings.workingHoursStart?.split(':')[0] || '8');
      ...
      for (let hour = startHour; hour < endHour; hour++) {
        for (let minute = 0; minute < 60; minute += slotDuration) {
          if (hour * 60 + minute + slotDuration <= endHour * 60) {
            ...
            const endHr = hour + Math.floor(endMinute / 60);
      ...
      // TODO: Filter out slots that are already booked or conflict with existing jobs
